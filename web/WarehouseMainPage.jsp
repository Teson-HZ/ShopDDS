<%-- 
    Document   : WarehouseMainPage
    Created on : 2024-3-15, 17:20:59
    Author     : WHZ2002
--%>
...
<div class="layui-layout layui-layout-admin">
  <div class="layui-header">
    <div class="layui-logo layui-hide-xs layui-bg-black">仓库功能栏</div>
    <!-- 头部区域 -->
    ...
    <ul class="layui-nav layui-layout-right" lay-filter="uppwd">
      ...
      <li class="layui-nav-item" style="color: #00FFFF;">${sessionScope.warehouseName}</li>
...
<div id="l1" class="layui-tab-item layui-card layui-panel" style="margin-top: 20px; background: #ffffff; border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    <div class="layui-card-header" style="background: linear-gradient(135deg, #3F51B5, #5C6BC0); border-radius: 15px 15px 0 0; color: white; font-size: 18px; font-weight: bold; padding: 20px; text-align: center;">
        查看所有商品信息
    </div>
    <div class="layui-card-body" style="padding: 20px; font-size: 14px; color: #333; background-color: #F7F9FC; line-height: 1.6;">
        <form class="layui-form layui-form-pane" action="SelectCommodityWarehouseServlet" method="post">
            <div class="layui-form-item" style="text-align: center;">
                <button class="layui-btn" type="submit" style="background: linear-gradient(135deg, #3F51B5, #5C6BC0); border-radius: 10px; border: none;">显示商品信息</button>
            </div>
        </form>
    </div>
</div>
...
<div id="l2" class="layui-tab-item layui-card layui-panel" style="margin-top: 20px; background: #ffffff; border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    <div class="layui-card-header" style="background: linear-gradient(135deg, #009688, #4CAF50); border-radius: 15px 15px 0 0; color: white; font-size: 18px; font-weight: bold; padding: 20px; text-align: center;">
        提交商品库存信息
    </div>
    <div class="layui-card-body" style="padding: 20px; font-size: 14px; color: #333; background-color: #F7F9FC; line-height: 1.6;">
        <form class="layui-form layui-form-pane" action="AddInvServlet" method="post">
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select id="commodityid1" name="commodity_Id" lay-verify="required" lay-filter="commodityid1" class="select" style="border-radius: 10px; border: 1px solid #E0E0E0;">
                        <option value=""></option>
                    </select>
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label" style="width: 160px;">输入可供分配数量</label>
                <div class="layui-input-inline">
                    <input type="text" name="inv_Amount" autocomplete="off" placeholder="请输入数量" lay-verify="required" class="layui-input" style="border-radius: 10px; border: 1px solid #E0E0E0;">
                </div>
            </div>
            <div class="layui-form-item" style="text-align: center;">
                <button class="layui-btn" type="submit" style="background: linear-gradient(135deg, #009688, #4CAF50); border-radius: 10px; border: none;">提交</button>
                <button type="reset" class="layui-btn layui-btn-primary" style="border-radius: 10px; border: 1px solid #E0E0E0;">重置</button>
            </div>
        </form>
        <%
            if (request.getAttribute("addinv") != null) {
                String addinv = (String) request.getAttribute("addinv");
                if (!addinv.isEmpty()) { // 只有在message不为空字符串时才弹出提示
                    %>
                    <script>
                        alert('<%= addinv %>');
                    </script>
                    <%
                    request.setAttribute("addinv", ""); // 提交表单后将message的值设置为空字符串
                }
            }
        %>
    </div>
...
<div id="l4" class="layui-tab-item layui-card layui-panel" style="margin-top: 20px; background: #ffffff; border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    <div class="layui-card-header" style="background: linear-gradient(135deg, #FF5722, #FF7043); border-radius: 15px 15px 0 0; color: white; font-size: 18px; font-weight: bold; padding: 20px; text-align: center;">
        查看本仓库发物表
    </div>
    <div class="layui-card-body" style="padding: 20px; font-size: 14px; color: #333; background-color: #F7F9FC; line-height: 1.6;">
        <form class="layui-form layui-form-pane" action="SelectDistributeServlet" method="post">
            <div class="layui-form-item" style="text-align: center;">
                <button class="layui-btn" type="submit" style="background: linear-gradient(135deg, #FF5722, #FF7043); border-radius: 10px; border: none;">点击查询</button>
            </div>
        </form>
    </div>
</div>
...
<div id="l9" class="layui-tab-item layui-card layui-panel" style="margin-top: 20px; background: #ffffff; border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    ...
    <div class="layui-card-body" style="padding: 20px; font-size: 14px; color: #333; background-color: #F7F9FC; line-height: 1.6;">
        <form class="layui-form layui-form-pane" action="UpdatePwd2Servlet" method="post">
            <div class="layui-form-item">
                <label class="layui-form-label" style="white-space: nowrap; overflow: hidden; text-overflow: clip; width: 160px;">请输入原密码</label>
                <div class="layui-input-inline">
                    <input type="password" name="SoursePwd" autocomplete="off" placeholder="请输入原密码" required lay-verify="required" class="layui-input" style="border-radius: 10px; border: 1px solid #E0E0E0;">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label" style="white-space: nowrap; overflow: hidden; text-overflow: clip; width: 160px;">请输入新密码</label>
                <div class="layui-input-inline">
                    <input type="password" name="NewPwd" autocomplete="off" placeholder="请输入新密码" required lay-verify="required" class="layui-input" style="border-radius: 10px; border: 1px solid #E0E0E0;">
                </div>
            </div>
            <div class="layui-form-item" style="text-align: center;">
                <button class="layui-btn" type="submit" style="background: linear-gradient(135deg, #3F51B5, #5C6BC0); border-radius: 10px; border: none;">提交</button>
                <button type="reset" class="layui-btn layui-btn-primary" style="border-radius: 10px; border: 1px solid #E0E0E0;">重置</button>
            </div>
        </form>
        <%
            if (request.getAttribute("UpD2") != null) {
                String UpD = (String) request.getAttribute("UpD2");
                if (!UpD.isEmpty()) { // 只有在message不为空字符串时才弹出提示
                    ...
                    <%
                    request.setAttribute("UpD2", ""); // 提交表单后将message的值设置为空字符串
                }
            }
        %>
    </div>
...
layui.use(['element', 'layer'], function(){
  ...
  element.on('nav(uppwd)', function(elem){
    var id4 = elem.text().trim(); // 获取点击的导航项文本
    // 根据导航项文本显示对应的内容
    switch (id4) {
      case '修改密码':
        showContent('l9');
...
layui.use(['form', 'jquery'], function(){
    ...
    function loadSelectData() {
        $.ajax({
            ...
            success: function (data) {
                $('#commodityid1').empty(); // 先清空下拉菜单
                $('#commodityid1').append('<option value=""></option>'); // 添加默认空选项
                $.each(data, function(index, item){
                    $('#commodityid1').append(new Option(item.id + ' - ' + item.name, item.id)); // 添加选项，这里假设你想显示 name 而不是 id
                });
    ...
    form.on('select(commodityid1)', function(data){
