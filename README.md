...
2. 在 MySQL 中创建数据库并执行 `/sql/schema.sql`（包含全部建表语句）。（请检查编码与用户权限）
3. 修改后端配置（`/config` 或 `db.properties`）：填写 DB 连接、用户名、密码、JDBC URL。
4. 在 IDE 中导入 Web 项目（或打包为 `.war`）：使用 Maven / Ant（视项目结构而定）。
5. 将 `.war` 部署到 Tomcat，访问：`http://<host>:<port>/shopdds/`（入口页面与登录界面在文档第六章有示例截图）。
...
该 README 基于项目设计文档《连锁超市物资分配调拨决策支持系统》（ShopDDS.docx）整理，文档包含需求分析、总体设计、详细设计、实现与测试等完整内容。
